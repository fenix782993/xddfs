...
CREATE TABLE IF NOT EXISTS mines_rounds (
    ...
    mine_positions JSONB NOT NULL DEFAULT '[]',
...
DEFAULT_GAMES = [
    ...
    ("high_low", "⬆️ High / Low", "Выбери высокий или низкий результат", "⬆️"),
    ("rps", "✊ RPS", "Камень, ножницы, бумага", "✊"),
    ("color", "🌈 Color", "Выбери цвет", "🌈"),
]
...
async def init_db():
    ...
    async with pool.acquire() as conn:
        ...
        migrations = [
            ...
            "ALTER TABLE achievements ADD COLUMN IF NOT EXISTS description TEXT DEFAULT ''",
            ...
            "ALTER TABLE missions ADD COLUMN IF NOT EXISTS target TEXT DEFAULT ''",
            ...
            "ALTER TABLE mines_rounds ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'active'",
        ]
...
async def create_mines_round(user_id: int, bet: int, mines: int, positions: list[int]):
    db = check_pool()
    async with db.acquire() as conn:
        async with conn.transaction():
            user = await conn.fetchrow("SELECT balance, banned FROM users WHERE id=$1 FOR UPDATE", int(user_id))
            if not user: raise ValueError("user_not_found")
            if user["banned"]: raise ValueError("user_banned")
            if int(user["balance"]) < int(bet): raise ValueError("insufficient_funds")
            before = int(user["balance"]); after = before - int(bet)
            await conn.execute("UPDATE users SET balance=$2, updated_at=NOW() WHERE id=$1", int(user_id), after)
            await conn.execute("INSERT INTO transactions(user_id,amount,balance_before,balance_after,reason,meta) VALUES($1,$2,$3,$4,'mines_lock',$5::jsonb)", int(user_id), -int(bet), before, after, json_dump({"mines": mines}))
            return await conn.fetchrow("INSERT INTO mines_rounds(user_id,bet,mines,mine_positions) VALUES($1,$2,$3,$4::jsonb) RETURNING *", int(user_id), int(bet), int(mines), json_dump(positions))

async def get_active_mines_round(user_id: int):
    db = check_pool()
    return await db.fetchrow("SELECT * FROM mines_rounds WHERE user_id=$1 AND status='active' ORDER BY id DESC LIMIT 1", int(user_id))

async def update_mines_round(round_id: int, opened: list[int], multiplier: float):
    db = check_pool()
    return await db.fetchrow("UPDATE mines_rounds SET opened=$2::jsonb,multiplier=$3 WHERE id=$1 AND status='active' RETURNING *", int(round_id), json_dump(opened), float(multiplier))

async def finish_mines_round(round_id: int, user_id: int, win_amount: int, multiplier: float, hit_mine: bool):
    db = check_pool()
    async with db.acquire() as conn:
        async with conn.transaction():
            row = await conn.fetchrow("SELECT * FROM mines_rounds WHERE id=$1 AND user_id=$2 AND status='active' FOR UPDATE", int(round_id), int(user_id))
            if not row: raise ValueError("round_not_active")
            user = await conn.fetchrow("SELECT balance FROM users WHERE id=$1 FOR UPDATE", int(user_id))
            before = int(user["balance"]); after = before + int(win_amount)
            if win_amount:
                await conn.execute("UPDATE users SET balance=$2,updated_at=NOW() WHERE id=$1", int(user_id), after)
                await conn.execute("INSERT INTO transactions(user_id,amount,balance_before,balance_after,reason,meta) VALUES($1,$2,$3,$4,'mines_payout',$5::jsonb)", int(user_id), int(win_amount), before, after, json_dump({"round_id": round_id}))
            await conn.execute("UPDATE mines_rounds SET status=$2,finished_at=NOW(),multiplier=$3 WHERE id=$1", int(round_id), "lost" if hit_mine else "finished", float(multiplier))
            bet = int(row["bet"]); profit = int(win_amount) - bet
            opened = row["opened"]
            if isinstance(opened, str): opened = json.loads(opened)
            await conn.execute("UPDATE users SET games=games+1,wins=wins+$2,losses=losses+$3,last_activity_at=NOW(),updated_at=NOW() WHERE id=$1", int(user_id), 1 if profit > 0 else 0, 1 if profit <= 0 else 0)
            await conn.execute("INSERT INTO game_history(user_id,game_code,bet,win,profit,multiplier,result) VALUES($1,'mines',$2,$3,$4,$5,$6::jsonb)", int(user_id), bet, int(win_amount), profit, float(multiplier), json_dump({"round_id": round_id, "opened": opened, "hit_mine": hit_mine}))
            return after
